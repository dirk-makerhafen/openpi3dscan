...
class Animation(GenericTask):
    ...
    def _screenshots_to_animation(self, path, output_file, filetype):
        ...
        framerate = int(round(100 / (total_duration / len(files)), 0))
        if filetype == "mp4":
            subprocess.check_output(shlex.split('"%s" -f image2 -framerate %s -i "%s\\screenshot_%%3d.png" -vcodec libx264 -vf scale=1200x1018 -crf 10 -pix_fmt yuv420p "%s" ' % (ExternalFilesInstance().ffmpeg, framerate, path, output_file)), shell=False, creationflags=CREATE_NO_WINDOW)
        else:
            try:
