...
class ShotsItemView(PyHtmlView):
    ...
    <div class="row {% if pyview.parent.selected_shot.shot_id == pyview.subject.shot_id %} selected {% endif %}" onclick='pyview.parent.select_shot("{{pyview.subject.shot_id}}");'>
        <div class="col-md-12">
            ...
            <div class="info"> 
                {{pyview.subject.nr_of_files}},{{pyview.subject.nr_of_models}}{% if pyview.subject.nr_of_models_waiting_or_processing > 0 or pyview.subject.nr_of_models_failed > 0 %}({% if pyview.subject.nr_of_models_waiting_or_processing > 0%}{{pyview.subject.nr_of_models_waiting_or_processing}}{% endif %}{% if pyview.subject.nr_of_models_waiting_or_processing > 0 and pyview.subject.nr_of_models_failed > 0 %},{% endif %}{% if pyview.subject.nr_of_models_failed > 0%}{{pyview.subject.nr_of_models_failed}}!{% endif %}){% endif %} 
            </div>  
        </div>  
    </div>
    '''