...
class Processing(Observable):
    ...
    def _request_remote(self):
        ...
        data = self.get_unprocessed_models()
        ...
        for model in data["models"]:
            self.processing(model["shot_id"], model["model_id"])
            self._clean_shot_dir()
            self.set_status("download")
            shot_path = self.download_shot(model["shot_id"])
    ...
    def upload_calibration_data(self, data):
        ...
        try:
            requests.post("http://%s/upload_calibration" % (SERVER), json={"data": json.dumps(data)})
            print("Upload finished")
        except:
            print("failed to reach server while uploading")
