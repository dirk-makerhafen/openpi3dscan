...
class Device(Observable):
    ...
    def _deploy(self):
        ...
        self._ssh('cd /home/%s/client ; sudo python3 run.py install "%s" "%s" "%s" "%s" "install_after_reboot"' % (self.username, self.device_id, self.device_type, self.name, rotation), timeout=180)
        self.latest_heartbeat_time = 0
